(function() {
  ...
  function ControlController($scope,$state) {
    ...
    $scope.ToggleSwitch =  function(item){
      if(item.Value){
        console.log("OFF");
        $.get('http://'+Host+'/index.php/Actions/TurnOffControl/'+item.ID);
      }
      else{
        console.log("ON");
        $.get('http://'+Host+'/index.php/Actions/TurnOnControl/'+item.ID);
      }
    ...
    $scope.removeControl =  function(){
      if(confirm("are u sure?")){
        $.post(
          'http://'+Host+'/index.php/Actions/RemoveControl/',
          {
            'ID' : this.item.ID
          },
    ...
    $scope.addNew = function(){
      $scope.addmode = false;
      $.post(
        'http://'+Host+'/index.php/Actions/AddControl/',
        {
    ...
    $scope.saveEdit = function(){
      this.item.editmode = false;
      $.post(
        'http://'+Host+'/index.php/Actions/EditControl/',
        {
          'ID' : this.item.ID,
          'Name' : this.item.Name,
          'Type' : this.item.Type
    ...
    function GetData(){
      $.getJSON('http://'+Host+'/index.php/Queries/GetAllControls', function(jd) {
          $scope.items = jd;
          $scope.$digest();
      });
    }
